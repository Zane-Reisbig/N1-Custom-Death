...
exports.SPTStartLocalRaid = void 0;
const enums_1 = require("../Definitions/enums");
class SPTStartLocalRaid {
    static helpers;
    static onRaidStart;
    generateLocationAndLoot = (name, generateLoot) => {
        ...
        SPTStartLocalRaid.helpers.pmcWaveGenerator.applyWaveChangesToMap(locationBaseClone);
        // Adjust raid based on whether this is a scav run
        let locationConfigClone;
        const raidAdjustments = SPTStartLocalRaid.helpers.applicationContext
            .getLatestValue(enums_1.ContextVariableType.RAID_ADJUSTMENTS)
            ?.getValue();
        if (raidAdjustments) {
            locationConfigClone = SPTStartLocalRaid.helpers.cloner.clone(SPTStartLocalRaid.helpers.locationConfig); // Clone values so they can be used to reset originals later
            SPTStartLocalRaid.helpers.raidTimeAdjustmentService.makeAdjustmentsToMap(raidAdjustments, locationBaseClone);
        }
        const staticAmmoDist = SPTStartLocalRaid.helpers.cloner.clone(location.staticAmmo);
        // Create containers and add loot to them
        const staticLoot = SPTStartLocalRaid.helpers.locationLootGenerator.generateStaticContainers(locationBaseClone, staticAmmoDist);
        locationBaseClone.Loot.push(...staticLoot);
        // Add dynamic loot to output loot
        const dynamicLootDistClone = SPTStartLocalRaid.helpers.cloner.clone(location.looseLoot);
        const dynamicSpawnPoints = SPTStartLocalRaid.helpers.locationLootGenerator.generateDynamicLoot(dynamicLootDistClone, staticAmmoDist, name.toLowerCase());
        ...
        SPTStartLocalRaid.helpers.logger.success(SPTStartLocalRaid.helpers.localisationService.getText("location-dynamic_items_spawned_success", dynamicSpawnPoints.length));
        SPTStartLocalRaid.helpers.logger.success(SPTStartLocalRaid.helpers.localisationService.getText("location-generated_success", name));
        // Reset loot multipliers back to original values
        if (raidAdjustments && locationConfigClone) {
            SPTStartLocalRaid.helpers.logger.debug("Resetting loot multipliers back to their original values");
            ...
            SPTStartLocalRaid.helpers.applicationContext.clearValues(enums_1.ContextVariableType.RAID_ADJUSTMENTS);
        }
        return locationBaseClone;
    };
    adjustBotHostilitySettings = (location) => {
        for (const botId in SPTStartLocalRaid.helpers.pmcConfig.hostilitySettings) {
            const configHostilityChanges = SPTStartLocalRaid.helpers.pmcConfig.hostilitySettings[botId];
            const locationBotHostilityDetails = location.BotLocationModifier.AdditionalHostilitySettings.find((botSettings) => botSettings.BotRole.toLowerCase() === botId);
            // No matching bot in config, skip
            if (!locationBotHostilityDetails) {
                SPTStartLocalRaid.helpers.logger.warning(`No bot: ${botId} hostility values found on: ${location.Id}, can only edit existing. Skipping`);
                continue;
            }
            ...
            if (configHostilityChanges.chancedEnemies) {
                ...
                for (const chanceDetailsToApply of configHostilityChanges.chancedEnemies) {
                    const locationBotDetails = locationBotHostilityDetails.ChancedEnemies.find((botChance) => botChance.Role === chanceDetailsToApply.Role);
    ...
    adjustExtracts = (playerSide, location, locationData) => {
        const playerIsScav = playerSide.toLowerCase() === "savage";
        if (playerIsScav) {
            // Get relevant extract data for map
            const mapExtracts = SPTStartLocalRaid.helpers.databaseService.getLocation(location)?.allExtracts;
            if (!mapExtracts) {
                SPTStartLocalRaid.helpers.logger.warning(`Unable to find map: ${location} extract data, no adjustments made`);
                return;
            }
    ...
    startLocalRaid = (sessionId, request) => {
        SPTStartLocalRaid.helpers.logger.debug(`Starting: ${request.location}`);
        const playerProfile = SPTStartLocalRaid.helpers.profileHelper.getPmcProfile(sessionId);
        ...
        const result = {
            serverId: `${request.location}.${request.playerSide}.${SPTStartLocalRaid.helpers.timeUtil.getTimestamp()}`, // TODO - does this need to be more verbose - investigate client?
            serverSettings: SPTStartLocalRaid.helpers.databaseService.getLocationServices(), // TODO - is this per map or global?
            ...
            transitionType: enums_1.TransitionType.None,
            transition: {
                transitionType: enums_1.TransitionType.None,
        ...
        const transitionData = SPTStartLocalRaid.helpers.applicationContext
            .getLatestValue(enums_1.ContextVariableType.TRANSIT_INFO)
            ?.getValue();
        if (transitionData) {
            SPTStartLocalRaid.helpers.logger.success(`Player: ${sessionId} is in transit to ${request.location}`);
            result.transition.transitionType = enums_1.TransitionType.Common;
            ...
            SPTStartLocalRaid.helpers.applicationContext.clearValues(enums_1.ContextVariableType.TRANSIT_INFO);
        }
...
exports.SPTStartLocalRaid = SPTStartLocalRaid;
