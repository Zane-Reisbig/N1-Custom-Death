...
export default class Source {
    ...
    public static setPlayerInventoryOnDeath: (
        playerPMC: IPmcData,
        sessionID: string
    ) => void;
    ...
    migrateScavQuestProgressToPmcProfile(
        ...
        for (const scavQuest of scavProfile.Quests) {
            ...
            if (!pmcQuest) {
                Source.helpers.logger.warning(
                    Source.helpers.localisationService.getText(
    ...
    handlePostRaidPlayerScav(
        ...
        if (isTransfer) {
            ...
            Source.helpers.inRaidHelper.setInventory(
    ...
    processAchievementRewards(
        ...
        for (const achievement of achievements) {
            const rewardItems = Source.helpers.rewardHelper.applyRewards(
                achievement.rewards,
            ...
            if (rewardItems?.length > 0) {
                Source.helpers.mailSendService.sendLocalisedSystemMessageToPlayer(
                    sessionId,
                    "670547bb5fa0b1a7c30d5836 0",
                    rewardItems,
                    [],
                    Source.helpers.timeUtil.getHoursAsSeconds(24 * 7)
                );
            }
        }
    }

    processPostRaidQuests(questsToProcess: IQuestStatus[]): IQuestStatus[] {
    ...
    lightkeeperQuestWorkaround(
        ...
        for (const questToComplete of newlyCompletedLightkeeperQuests) {
            Source.helpers.questHelper.completeQuest(
                pmcProfile,
                {
    ...
    handleInsuredItemLostEvent(
        ...
        if (request.lostInsuredItems?.length > 0) {
            const mappedItems = Source.helpers.insuranceService.mapInsuredItemsToTrader(
                sessionId,
            ...
            Source.helpers.insuranceService.storeGearLostInRaidToSendLater(
                sessionId,
                mappedItems
            );

            Source.helpers.insuranceService.startPostRaidInsuranceLostProcess(
                preRaidPmcProfile,
                sessionId,
                locationName
            );
    ...
    checkForAndFixPickupQuestsAfterDeath(
        ...
        const questDb = Object.values(Source.helpers.databaseService.getQuests()).filter(
            (quest) => activeQuestIdsInProfile.includes(quest._id)
        );
        ...
        for (const lostItem of lostQuestItems) {
            ...
            const profileQuestToUpdate = profileQuests.find(
    ...
    handleCarExtract(extractName: string, pmcData: IPmcData, sessionId: string): void {
        ...
        const newFenceStanding = this.getFenceStandingAfterExtract(
            pmcData,
            Source.helpers.inRaidConfig.carExtractBaseStandingGain,
        ...
        const scavData: IPmcData = Source.helpers.profileHelper.getScavProfile(sessionId);
        scavData.TradersInfo[fenceId].standing = pmcData.TradersInfo[fenceId].standing;
        scavData.TradersInfo[fenceId].loyaltyLevel =
            pmcData.TradersInfo[fenceId].loyaltyLevel;
    }

    handleCoopExtract(sessionId: string, pmcData: IPmcData, extractName: string): void {
        ...
        const newFenceStanding = this.getFenceStandingAfterExtract(
            pmcData,
            Source.helpers.inRaidConfig.coopExtractBaseStandingGain,
    ...
    sendCoopTakenFenceMessage(sessionId: string): void {
        ...
        const loot = Source.helpers.lootGenerator.createRandomLoot(
            Source.helpers.traderConfig.fence.coopExtractGift
        );
        ...
        Source.helpers.mailSendService.sendLocalisedNpcMessageToPlayer(
            ...
            mailableLoot,
            Source.helpers.timeUtil.getHoursAsSeconds(
                Source.helpers.traderConfig.fence.coopExtractGift.giftExpiryHours
            )
        );
    }
    ...
    public endLocalRaid(sessionId: string, request: IEndLocalRaidRequestData) {
        ...
        if (playerStatusDetails.isTransfer && request.locationTransit) {
            ...
            Source.helpers.applicationContext.addValue(
    ...
    public setInventory(
        ...
        Source.helpers.sptInventoryHelper.removeItem(
            serverProfile,
        ...
        const postRaidInventoryItems =
            Source.helpers.itemHelper.findAndReturnChildrenAsItems(
        ...
        const postRaidQuestItems = Source.helpers.itemHelper.findAndReturnChildrenAsItems(
            postRaidProfile.Inventory.items,
    ...
    addItemsToInventory(itemsToAdd: IItem[], serverInventoryItems: IItem[]): void {
    ...
    postRaidPlayerUSEC(
        ...
        const lostQuestItems =
            Source.helpers.profileHelper.getQuestItemsInProfile(postRaidProfile);
        ...
        Source.helpers.healthHelper.updateProfileHealthPostRaid(
            pmcProfile,
        ...
        if (isDead) {
            ...
            Source.helpers.pmcChatResponseService.sendKillerResponse(
                sessionId,
                pmcProfile,
            ...
            if (Source.setPlayerInventoryOnDeath) {
                ...
                Source.helpers.inRaidHelper.removeFiRStatusFromItemsInContainer(
                    sessionId,
                    pmcProfile,
                    "SecuredContainer"
                );
        ...
        if (victims?.length > 0) {
            ...
            Source.helpers.pmcChatResponseService.sendVictimResponse(
                sessionId,
                victims,
                pmcProfile
            );
    ...
    public static validateMembers(cls: Source) {
        Source.helpers.logger.log(
            `setPlayerInventoryOnDeath Hook Status: ${
                Source.setPlayerInventoryOnDeath != null ? "Loaded" : "Undefined"
            }`,
            Source.setPlayerInventoryOnDeath == null ? "red" : "green"
        );
